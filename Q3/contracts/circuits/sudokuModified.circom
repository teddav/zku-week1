...
template sudoku() {
    ...
    component checkedPuzzle[9][9];
    component checkedSolution[9][9];
    
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            checkedPuzzle[i][j] = RangeProof(10);
            checkedPuzzle[i][j].in <== puzzle[i][j];
            checkedPuzzle[i][j].range[0] <== 0;
            checkedPuzzle[i][j].range[1] <== 9;

            checkedSolution[i][j] = RangeProof(10);
            checkedSolution[i][j].in <== solution[i][j];
            checkedSolution[i][j].range[0] <== 0;
            checkedSolution[i][j].range[1] <== 9;

            assert(checkedSolution[i][j].out == 1);
            assert(checkedPuzzle[i][j].out == 1);
